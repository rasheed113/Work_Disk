...
public:
    ...
    bool find(const std::string& id, TrashEntry& e) const override { auto it=data.find(id); if(it==data.end()) return false; e=it->second; return true; }
    std::vector<TrashEntry> list() const override { std::vector<TrashEntry> out; for(const auto& [id,e]:data) out.push_back(e); return out; }
    TrashClaimResult claimForRestore(const std::string& id, TrashTime now, TrashEntry& e) override { auto it=data.find(id); if(it==data.end()) return TrashClaimResult::NotFound; if(claimed.count(id)) return TrashClaimResult::Conflict; if(now>=it->second.expiresAt) return TrashClaimResult::Expired; claimed.insert(id); e=it->second; return TrashClaimResult::Claimed; }
    TrashClaimResult claimForDestruction(const std::string& id, TrashEntry& e) override { auto it=data.find(id); if(it==data.end()) return TrashClaimResult::NotFound; if(claimed.count(id)) return TrashClaimResult::Conflict; claimed.insert(id); e=it->second; return TrashClaimResult::Claimed; }
