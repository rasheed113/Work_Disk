...
public:
    ...
    ApprovalDecisionStatus commitDecisionIfPending(
        ...
        if (it == states.end()) {
            return ApprovalDecisionStatus::UnknownRequest;
        }
...
int main() {
    ...
    const auto approved = bot.receiveDecision({
        "APR-001", "CONTRACTOR-01", ApprovalDecision::Approved, "DEC-001"
    });
    ...
    const auto conflict = bot.receiveDecision({
        "APR-001", "CONTRACTOR-01", ApprovalDecision::Rejected, "DEC-002"
    });
    assert(conflict.decisionStatus == ApprovalDecisionStatus::DecisionConflict);
    assert(consumer.last.state.decision == ApprovalDecision::Approved);
    ...
    const auto spoofed = bot.receiveDecision({
        "APR-002", "ATTACKER", ApprovalDecision::Approved, "DEC-003"
    });
    assert(spoofed.decisionStatus == ApprovalDecisionStatus::ApproverMismatch);

    const auto invalid = bot.createWarning(
        ApprovalRequest{}
    );
