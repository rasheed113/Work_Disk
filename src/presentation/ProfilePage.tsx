...
export function ProfilePage({ identity }: { identity: AuthenticatedIdentity }): ReactElement {
  ...
  useEffect(() => {
    ...
    setBusy(true)
    setError('')
    void repository.getProfile(identity).then(value => {
  ...
  async function save(event: FormEvent) {
    ...
    setSaved(false)
    try {
      const value = await repository.saveProfile(identity, profile)
      setProfile(value)
      setSaved(true)
    } catch (cause) {
  ...
  return <section className="profile-page">
    <div className="profile-hero" style={profile.coverUrl ? { backgroundImage: `linear-gradient(180deg,rgba(4,12,8,.08),rgba(4,12,8,.92)),url(${profile.coverUrl})` } : undefined}>
      <div className="profile-hero-content">
        <div className="profile-avatar-wrap"><div className="profile-avatar">{profile.photoUrl ? <img src={profile.photoUrl} alt="Profile" /> : profile.fullName.slice(0, 1).toUpperCase() || identity.email.slice(0, 1).toUpperCase()}</div></div>
    ...
    <div className="profile-completion profile-panel">
      <div><span className="eyebrow">PROFILE COMPLETION</span><strong>{completion}%</strong></div>
      <div className="completion-track"><span style={{ width: `${completion}%` }} /></div>
      <p>{completion === 100 ? 'Your required profile information is complete.' : 'Complete the highlighted profile details to finish your Social profile.'}</p>
    </div>

    <form className="profile-panel profile-form" onSubmit={save}>
      <div className="profile-section-heading"><span className="eyebrow">IDENTITY</span><h2>Profile details</h2></div>
      ...
      <div className="profile-system-card">
        <div><span className="eyebrow">WORK_DISK ID</span><strong>{profile.wdId}</strong><small>System-generated and immutable. Copy it for Social search or sending.</small></div>
        <button type="button" className="profile-copy" onClick={() => void navigator.clipboard?.writeText(profile.wdId)}>Copy ID</button>
      </div>
